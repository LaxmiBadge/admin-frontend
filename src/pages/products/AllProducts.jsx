...
import { FaPlus, FaStar, FaTimes, FaSearch } from "react-icons/fa";

/**
 * AllProducts - Admin products listing with floating label actions on image (Option C)
 *
 * Requirements:
...
const AllProducts = () => {
  ...
  // --- Product card component inline for readability
  const ProductCard = ({ p }) => {
    const discountVal = getDiscountValue(p);
    const price = Number(p.basePrice ?? p.price ?? 0);
    const finalPrice = price - (price * (discountVal || 0)) / 100;
    const imageSrc = p.images?.[0] || p.image || "/no-image.png";
    ...
    return (
      <motion.article
        ...
        whileHover={{ y: -6 }}
        className="bg-white/80 rounded-2xl shadow-lg border border-white/30 overflow-hidden flex flex-col"
      >
        {/* IMAGE + floating labels */}
        <div className="relative group">
          <img src={imageSrc} alt={p.name} className="w-full h-64 object-cover" />

          {/* discount badge */}
          {discountVal > 0 && (
            <div className="absolute left-3 top-3 bg-gradient-to-r from-[#f3cd7b] to-[#010620] text-white px-3 py-1 rounded-full text-xs font-semibold shadow-xl">
          ...
          {/* For touch devices, show labels below the image area (always visible) */}
          <div className="md:hidden flex items-center justify-center gap-3 p-3 bg-white/90">
            <button onClick={() => setViewProduct(p)} className="px-3 py-1 rounded-full bg-white text-sm font-medium shadow">
              View
            </button>
            <button onClick={() => setEditProduct(p)} className="px-3 py-1 rounded-full bg-yellow-100 text-sm font-medium shadow">
              Edit
            </button>
            <button onClick={() => handleDelete(p._id)} className="px-3 py-1 rounded-full bg-red-100 text-sm font-medium shadow">
              Delete
            </button>
          </div>
        </div>
        ...
        <div className="p-4 flex flex-col gap-2 flex-1">
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1">
              <h3 className="font-semibold text-[#062e57]">{p.name}</h3>
          ...
          <div className="mt-2 flex items-center justify-between">
            <div className={`text-sm font-medium ${p.stock > 0 ? "text-blue-950" : "text-blue-950"}`}>
              {p.stock > 0 ? `In stock (${p.stock})` : "Out of stock"}
            </div>

            <div className="hidden md:flex items-center gap-2">
              {/* small pill actions on desktop for quick access (redundant to floating labels) */}
              <button onClick={() => setViewProduct(p)} className="px-2 py-1 rounded-md bg-blue-950 text-white text-xs">View</button>
              <button onClick={() => setEditProduct(p)} className="px-2 py-1 rounded-md bg-orange-200 text-xs">Edit</button>
              <button onClick={() => handleDelete(p._id)} className="px-2 py-1 rounded-md bg-red-100 text-xs">Delete</button>
            </div>
          </div>

          <p className="text-xs text-gray-500 mt-3 line-clamp-3">{p.description}</p>
        </div>
      </motion.article>
    );
  };
  ...
  return (
    <AdminLayout>
      <div className="min-h-screen transition-all duration-300 bg-gradient-to-b from-[#f5f7fb] to-[#e8f0ff] py-10">
        <div className="max-w-7xl mx-auto px-6">
          {/* Header */}
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6">
            <div>
              <h1 className="text-3xl font-extrabold text-[#062e57] drop-shadow-sm">Products</h1>
              <p className="text-sm text-gray-500 mt-1">Premium management — modern UI with floating label actions.</p>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-[#5b8cff] to-[#010620] text-white shadow-lg transform hover:-translate-y-0.5 transition"
                title="Add product"
              >
                <FaPlus /> Add Product
              </button>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div className="bg-white/50 backdrop-blur-md border border-white/30 rounded-3xl p-5 shadow-lg">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex gap-2 mt-2">
                      <button onClick={() => setPage(1)} className="flex-1 px-3 py-2 rounded-lg border  bg-blue-950 text-white">Apply</button>
                  ...
                  <div className="bg-white/40 backdrop-blur rounded-2xl p-4 border border-white/20 shadow">
                    ...
                    <div className="flex flex-wrap gap-2">
                      <button
                        ...
                        className={`px-3 py-1 text-sm rounded-full border ${category === "" ? "bg-gradient-to-r from-[#5b8cff] to-[#010620] text-white" : "bg-white text-gray-700"}`}
                      >
                        All
                      </button>
                      {categories.slice(0, 8).map((c) => (
                        <button
                          ...
                          className={`px-3 py-1 text-sm rounded-full border ${category === c.category ? "bg-gradient-to-r from-[#5b8cff] to-[#010620] text-white" : "bg-white text-gray-700"}`}
                        >
                          {c.category}
                        </button>
                      ))}
            ...
            <main className="lg:col-span-9">
              {error && <div className="bg-red-50 text-blue-950 border border-red-100 p-3 rounded-md mb-6">{error}</div>}
                ...
                <>
                  ...
                  <div className="flex items-center justify-center gap-3 mt-8">
                    ...
                    {Array.from({ length: totalPages }).map((_, i) => (
                      <button key={i} onClick={() => setPage(i + 1)} className={`px-3 py-2 rounded-full border ${page === i + 1 ? "bg-gradient-to-r from-[#5b8cff] to-[#010620] text-white" : "bg-white/70"}`}>
                        {i + 1}
                      </button>
                    ))}
          ...
          <AnimatePresence>
            {viewProduct && (
              ...
              >
                ...
                <motion.div className="relative w-full max-w-3xl mx-4 bg-white rounded-3xl shadow-xl p-6" variants={modalBox} initial="hidden" animate="visible" exit="exit">
                  <h2 className="text-xl font-semibold mb-4 text-gray-800">{viewProduct.name}</h2>
                  <p className="text-gray-600 mb-4 line-clamp-3">{viewProduct.description}</p>

                  <div className="flex justify-end gap-3">
                    <button onClick={() => navigate(`/admin/products/view/${viewProduct._id}`)} className="px-4 py-2 rounded-md bg-gradient-to-r from-[#5b8cff] to-[#010620] text-white hover:scale-105 transition">View Details</button>
                    <button onClick={() => setViewProduct(null)} className="px-4 py-2 rounded-md bg-gray-200 text-gray-700 hover:scale-105 transition">Close</button>
                  </div>
                </motion.div>
              </motion.div>
            )}
            ...
            {showAddModal && (
              <motion.div className="fixed inset-0 z-50 flex items-center justify-center" variants={modalBackdrop} initial="hidden" animate="visible" exit="exit">
                ...
                <motion.div className="relative w-full max-w-3xl mx-4" variants={modalBox} initial="hidden" animate="visible" exit="exit">
                  <div className="bg-blue-950 rounded-3xl p-4 shadow-2xl border border-blue-950/30">
                    <AddProduct
                      onSaved={(p) => {
                        handleSaved(p);
